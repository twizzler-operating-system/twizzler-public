...
$(BUILDDIR)/us/data/%.sctx.obj: $(BUILDDIR)/us/data/%.sctx $(BUILDDIR)/utils/file2obj
	@echo "[OBJ] $@"
	@mkdir -p $(BUILDDIR)/us/data
	@$(BUILDDIR)/utils/file2obj -i $< -o $@ -p R

$(BUILDDIR)/us/data/login.sctx.obj: $(BUILDDIR)/us/data/login.sctx $(BUILDDIR)/utils/file2obj $(BUILDDIR)/us/data/login-key.pub.obj
	@echo "[OBJ] $@"
	@mkdir -p $(BUILDDIR)/us/data
	@$(BUILDDIR)/utils/file2obj -i $< -o $@ -p R -k $$($(BUILDDIR)/utils/objstat -i $(BUILDDIR)/us/data/login-key.pub.obj)

$(BUILDDIR)/us/data/%.sctx.tmp: $(BUILDDIR)/utils/file2obj
	@echo "[OBJ] $@"
	@mkdir -p $(BUILDDIR)/us/data
	@$(BUILDDIR)/utils/file2obj -i /dev/null -o $@ -p R

$(BUILDDIR)/us/data/login.sctx: $(BUILDDIR)/us/data/_login.sctx.tmp $(BUILDDIR)/us/data/bob.sctx $(BUILDDIR)/utils/sctx $(BUILDDIR)/utils/mkcap $(BUILDDIR)/utils/appendobj $(BUILDDIR)/us/data/login-rk.pem $(BUILDDIR)/us/login/login.text.obj
	@echo "[SCTX] $@"
	@mkdir -p $(BUILDDIR)/us/data
...
$(BUILDDIR)/us/data/init.sctx: $(BUILDDIR)/us/data/_init.sctx.tmp $(BUILDDIR)/us/data/login.sctx.obj $(BUILDDIR)/utils/sctx $(BUILDDIR)/utils/mkcap $(BUILDDIR)/utils/appendobj $(BUILDDIR)/us/data/login-rk.pem $(BUILDDIR)/us/data/init-rk.pem $(BUILDDIR)/us/init/init.text.obj
	@echo "[SCTX] $@"
	@mkdir -p $(BUILDDIR)/us/data
	LID=$$($(BUILDDIR)/utils/objstat -i $(BUILDDIR)/us/data/_init.sctx.tmp);\
	( \
...
$(BUILDDIR)/us/login/login.data.obj $(BUILDDIR)/us/login/login.text.obj: $(BUILDDIR)/us/login/login $(BUILDDIR)/us/login/login.flags $(UTILS) $(BUILDDIR)/us/data/login-key.pub.obj
	...
	FLAGS=$$(cat $<.flags);\
	$(BUILDDIR)/utils/file2obj -i $<.text -o $<.text.obj -p H -k $$($(BUILDDIR)/utils/objstat -i $(BUILDDIR)/us/data/login-key.pub.obj) -f 1:RWD:$$DATAID $$FLAGS
...
$(BUILDDIR)/us/init/init.data.obj $(BUILDDIR)/us/init/init.text.obj: $(BUILDDIR)/us/init/init $(BUILDDIR)/us/init/init.flags $(UTILS) $(BUILDDIR)/us/data/init-key.pub.obj
	...
	@DATAID=$$($(BUILDDIR)/utils/objstat -i $<.data.obj) ;\
	FLAGS=$$(cat $<.flags);\
	$(BUILDDIR)/utils/file2obj -i $<.text -o $<.text.obj -p H -k $$($(BUILDDIR)/utils/objstat -i $(BUILDDIR)/us/data/init-key.pub.obj) -f 1:RWD:$$DATAID $$FLAGS
