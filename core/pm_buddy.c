...
static uintptr_t __do_pmm_buddy_allocate(struct memregion *reg, size_t length)
{
	...
	if(list_empty(&reg->alloc->freelists[order])) {
		...
		struct list *elem1 = mm_ptov(a + reg->start);
		struct list *elem2 = mm_ptov(a + length + reg->start);

		list_insert(&reg->alloc->freelists[order], elem1);
		list_insert(&reg->alloc->freelists[order], elem2);
	}

	uintptr_t address = mm_vtop(list_pop(&reg->alloc->freelists[order])) - reg->start;
...
static int deallocate(struct memregion *reg, uintptr_t address, int order)
{
	...
	if(!bitmap_test(reg->alloc->bitmaps[order], bit)) {
		...
		if(!bitmap_test(reg->alloc->bitmaps[order], buddy_bit)) {
			struct list *elem = mm_ptov(buddy + reg->start);
			list_remove(elem);
			deallocate(reg, buddy > address ? address : buddy, order + 1);
		} else {
			struct list *elem = mm_ptov(address + reg->start);
...
void pmm_buddy_deallocate(struct memregion *reg, uintptr_t address)
{
	...
	int order = deallocate(reg, address - reg->start, 0);
	if(order >= 0) {
...
{
	char *start = reg->alloc->static_bitmaps = mm_ptov(reg->start);
	size_t bmlen = ((reg->length / MIN_SIZE) / 8) * 2 + MAX_ORDER * 2; /* 1/2 + 1/4 + 1/8 ... */
	long length = ((reg->length / MIN_SIZE) / (8));
	for(int i = 0; i <= MAX_ORDER; i++) {
		...
		start += length + 2;
		length /= 2;
		reg->alloc->num_allocated[i] = buddy_order_max_blocks(reg->length, i);
	}
	for(uintptr_t addr = reg->start + align_up(bmlen, MIN_SIZE); addr < reg->start + reg->length;
	    addr += MIN_SIZE) {
