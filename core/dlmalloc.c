			...
			if(to_reserve > 0) {
				...
				if(remaining_commit_size > 0) {
					...
					assert(0 < remaining_commit_size && remaining_commit_size % g_pagesize == 0);
					{
						...
						void *base_committed = VirtualAlloc(
						  g_last->top_committed, remaining_commit_size, MEM_COMMIT, PAGE_READWRITE);
				...
				}
				{
					...
					do {
						...
						assert(0 < reserve_size && reserve_size % g_regionsize == 0);
						/* Try to reserve this */
						base_reserved = VirtualAlloc(
						  memory_info.BaseAddress, reserve_size, MEM_RESERVE, PAGE_NOACCESS);
						if(!base_reserved) {
							int rc = GetLastError();
