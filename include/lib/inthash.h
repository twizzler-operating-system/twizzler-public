...
#define ihtable_size(bits) (sizeof(struct ihtable) + ((1ul << (bits)) + 1) * sizeof(struct ihelem))
...
__attribute__((used)) static void _iht_ctor(void *sz, void *obj)
{
...
{
	assertmsg((unsigned)bits < sizeof(size_t) * 8, "bits = %d", bits);
...
#define ihtable_find(t, key, type, memb, keymemb)                                                  \
	({                                                                                             \
		type *ret = NULL;                                                                          \
		int bucket = sizeof(key) > 8 ? hash128_sz((key), (t)->bits) : hash64_sz((key), (t)->bits); \
